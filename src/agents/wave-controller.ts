      ...
      return async (): Promise<void> => {
        try {
          ...
          if (result.success) {
            ...
            // DISABLED 2026-09-20. This block wrote one Insight-classed node
            // per document whose entire content was boilerplate — "Insight
            // document generated by wave-analysis for Foo (Detail). Source
            // document: /…/Foo.md" — 153-244 chars, all 128 identical in shape,
            // while the document itself held ~9KB of real analysis.
            //
            // It added nothing the graph did not already have. The REAL entity
            // is one hop in (`Foo`, the Detail/SubComponent this insight is
            // about) and already carries the genuine description AND
            // `has_insight_document` / `validated_file_path` pointing at the
            // same document. The satellite contributed a filename.
            //
            // It existed only because HistorySidebar filters to
            // `entityType === 'Insight'` and UKB writes Components/
            // SubComponents/Details — so the wave manufactured nodes of the
            // class the sidebar wanted. That is fitting the knowledge base to a
            // view. The filter was widened instead (HistorySidebar now includes
            // rows carrying `metadata.has_insight_document`), which surfaces the
            // real entity, with its real content, and costs the graph nothing.
            //
            // Do not re-enable without deciding what an Insight NODE should
            // contain that its subject entity does not already say. If the
            // answer is "the document's substance", then write the substance —
            // not a pointer to it.
            const PERSIST_INSIGHT_ENTITIES = false;
            try {
              if (PERSIST_INSIGHT_ENTITIES && this.kmCoreAdapter) {
